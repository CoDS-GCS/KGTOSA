...
class KG(object):
    def __init__(self, args, path, ds):     
        ...
        for split in ['train', 'test', 'valid']:
            print('FILE = ',os.path.join(self.path, '{}.txt'.format(split)))
            i = 1
            for line in open(os.path.join(self.path, '{}.txt'.format(split))):
                # print('len = ',len(line.strip().split('\t')))
                try:
                    ...
                    pass
                    # print('Error at line ',split,' ',len(line.strip().split('\t')),' ',line)
