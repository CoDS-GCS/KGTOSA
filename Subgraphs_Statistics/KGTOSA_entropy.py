...
node_types_per_hop= {}
edge_types_per_hop= {}
def neighbour_type_distribution(G):
    types_counts_dict={}
    for n in nx_g.nodes.items():
        neighbours_lst = list(nx_g.neighbors(n[0]))
        node_type_count = len(set([str(x).split("/")[3] for x in neighbours_lst]))
        if node_type_count in types_counts_dict:
            types_counts_dict[node_type_count]+=1
        else:
            types_counts_dict[node_type_count] =1
    Pk = np.zeros(len(types_counts_dict))
    mapping_dict=dict(zip(list(types_counts_dict.keys()),list(range(0,len(types_counts_dict)))))# P(k)
    for k in types_counts_dict:
        Pk[mapping_dict[k]] = types_counts_dict[k]
    Pk = Pk / sum(types_counts_dict.values())
    return list(types_counts_dict.values()), Pk
def degree_distribution(G):
    ...
    return kvalues, Pk
def shannon_entropy(G,dist_type="node_degree"):
    if dist_type=="node_degree":
        k,Pk = degree_distribution(G)
    elif dist_type=="neighbour_type":
        k,Pk = neighbour_type_distribution(G)
    H = 0
    for p in Pk:
        if(p > 0):
            H = H - p*math.log(p, 2)
...
def calc_single_source_shortest_path_length(nx_g,target_nodes,src):
    ...
    distances=[v for k, v in dic_length.items() if k in target_nodes and v>0]
...
if __name__ == '__main__':
    datasets={
              # "YAGO_FM": {"target_node": "CreativeWork", "target_node_type_id": 1, "samplers": ["YAGO_CW_BRW_GS_DecodedSubgraph_20000TN", "YAGO_CW_URW_GS_DecodedSubgraph_20000TN","YAGO_CW_d1h1_GS_DecodedSubgraph_20000TN", "ibmb_YAGO30M_CW_subgraphs_20000PN"],'path': '/media/hussein/UbuntuData/GithubRepos/ogb_cods/examples/nodeproppred/mag/Sampled_Subgraphs/YAGO_WG_Samples_20000TN/'},
              "YAGO_Place_FM": {"target_node": "Place", "target_node_type_id": 1,"samplers": ["YAGO_PC_FM50_BiasedGS_DecodedSubgraph", "YAGO_PC_FM50_DecodedSubgraph"],'path': "/media/hussein/UbuntuData/GithubRepos/ogb_cods/examples/nodeproppred/mag/YAGO_PC_FM50_DecodedSubgraph/"},
              # "MAG42M_PV_FG":{"target_node":"Paper","target_node_type_id":64,"samplers": ["OGBN-MAG_PV_URW", "OGBN-MAG_PV_BRW","OGBN-MAG_PV_URW_d1h1", "ibmb_mag42M_subgraph_20000PN"],'path': '/media/hussein/UbuntuData/GithubRepos/ogb_cods/examples/nodeproppred/mag/Sampled_Subgraphs/OGBN-MAG_PV_Samples_20000TN/'},
              ...
              # "DBLP15M_PV_FG":{"target_node":"rec","target_node_type_id":38,"samplers":["DBLP_PV_BRW_GS_DecodedSubgraph_20TN","DBLP_PV_d1h1_GS_DecodedSubgraph","DBLP_PV_URW_GS_DecodedSubgraph_20TN","ibmb_DBLP15M_PV_subgraphs_20PN"],'path':'/media/hussein/UbuntuData/GithubRepos/ogb_cods/examples/nodeproppred/mag/Sampled_Subgraphs/DBLP_PV_Samples_20TN/'}
            # "DBLP15M_PV_FG": {"target_node": "rec", "target_node_type_id": 38,"samplers": ["DBLP_FG_BiasedGS_DecodedSubgraph", "DBLP_FG_DecodedSubgraph","DBLP_SQ_GS_DecodedSubgraph", "ibmb_DBLP15M_PV_subgraphs_20000PN"],'path': '/media/hussein/UbuntuData/GithubRepos/ogb_cods/examples/nodeproppred/mag/Sampled_Subgraphs/DBLP_PV_Samples_20000TN/'}
            }
    for k,v in datasets.items():
        ...
        for sampler in Samplers:
            ...
            ds_idx=0
            entropy_lst=[]
            dist_type = "neighbour_type"
            # dist_type = "node_degree"
            for idx in range(1,15):
                res.append({})
                # res[idx]={}
                file_name=sampler+"_"+str(idx) + ".csv"
                print("file_name=", file_name)
                file_path=v["path"]+sampler+"/"+file_name
                if os.path.isfile(file_path):
                    res[ds_idx]['name']=file_name
                    res[ds_idx]["target_node"]=target_node
                    # print("######################################################################################################")
                    ...
                    g_shannon_entropy=shannon_entropy(nx_g,dist_type)
                    print("g_shannon_entropy",dist_type,"=",g_shannon_entropy)
                    entropy_lst.append([file_name,g_shannon_entropy])
                    # num_nodes=len(nx_g.nodes)
                    # num_edges =len(nx_g.edges)
                    # target_nodes = {k for k in nx_g.nodes() if target_node in k}
                    # res[ds_idx]['target_nodes_count'] = len(target_nodes)
                    # res[ds_idx]['target_nodes_ratio'] = len(target_nodes) / num_nodes
                    ...
                    # # max_target_dist, avg_target_dist, num_disconnected, num_connected = AVG_NODE_DISTANCE_from_TARGET_Parallel(nx_g, target_nodes, non_target_nodes)
                    # res[ds_idx]['avg_target_dist']=avg_target_dist
                    # res[ds_idx]['max_target_dist'] = max_target_dist
                    # res[ds_idx]['num_target_disconnected'] = num_disconnected
                    # res[ds_idx]['num_target_connected'] = num_connected
                    # res[ds_idx]['disconnected_ratio'] = num_disconnected / num_nodes
                    # print("res[ds_idx]=",res[ds_idx])
                    # ds_idx += 1
                    # # print(res)
                    # df = pd.DataFrame(res)
                    # df.to_csv("BRW_WL_statisctics_" + sampler + ".tsv", sep="\t", index=None)
            pd.DataFrame(entropy_lst).to_csv(dist_type+"_entropy_" + sampler + ".tsv", sep="\t", index=None)
